...
function watchall(){
    ...
    watch(['src/js/*.js' , 'src/**/*.js'],ugjs);//監看js
}
...
function browser(done) {
    ...
    watch(['src/sass/*.scss', 'src/sass/**/*scss'] , sassstyle).on('change' , reload); //監看哪些檔案(檔案變動)並執行sassstyle
    watch(['src/*.html', 'src/layout/*.html'], includeHTML).on('change' , reload); // 監看哪些檔案（檔案變動）並執行includeHTML
    watch(['src/js/*.js' , 'src/**/*.js'],ugjs).on('change' , reload);//監看js
...
exports.package = series(clear, parallel(includeHTML, sassstyle, ugjs), imgmin);